...
class Social(Cog):
    ...
    async def player(self, interaction: Interaction, name: str) -> None:
        ...
        if not player:
            await interaction.response.send_message(
                embed=NameErrorEmbed(name), ephemeral=True
            )
            return
        ...
        embed.add_field(
            name="📚 Total fame",
            value=f"**{(player["KillFame"] + player["LifetimeStatistics"]["PvE"]["Total"] + player["LifetimeStatistics"]["Gathering"]["All"]["Total"] + player["LifetimeStatistics"]["Crafting"]["Total"] + player["LifetimeStatistics"]["FishingFame"] + player["LifetimeStatistics"]["FarmingFame"]):,}**",
            inline=False,
        )
        embed.add_field(
            name="🛠️ Crafting fame",
            value=f"**{player["LifetimeStatistics"]["Crafting"]["Total"]:,}**",
            inline=False,
        )

        embed.add_field(
            name="👾 PvE fame",
            value=f"**{player["LifetimeStatistics"]["PvE"]["Total"]:,}**",
        )
        embed.add_field(name="🗡️ Kill fame", value=f"**{player["KillFame"]:,}**")
        embed.add_field(name="💀 Death fame", value=f"**{player["DeathFame"]:,}**")

        for typee, emoji in zip(
            ("Fiber", "Hide", "Ore", "Rock", "Wood"), ("🌿", "🐘", "🍫", "🪨", "🌳")
        ):
            embed.add_field(
                name=f"{emoji} {typee} fame",
                value=f"**{player["LifetimeStatistics"]["Gathering"][typee]["Total"]:,}**",
            )
        embed.add_field(
            name="🐟 Fishing fame",
            value=f"**{player["LifetimeStatistics"]["FishingFame"]:,}**",
        )
    ...
    async def guild(self, interaction: Interaction, name: str) -> None:
        ...
        embed: Embed = Embed(
            title=f"🛡️ {guild["guild"]["Name"]}",
            color=Color.brand_green(),
            description="Use `/members` to get the list of guild members.",
        )
        embed.add_field(
            name=":man: Founder", value=f"**{guild["guild"]["FounderName"]}**"
        )
        if len(guild["guild"]["AllianceTag"]) != 0:
            embed.add_field(
                name="🤝 Alliance", value=f"**{guild["guild"]["AllianceTag"]}**"
            )
        ...
        embed.add_field(
            name="🗡️📚 Kill fame", value=f"**{guild["guild"]["killFame"]:,}**"
        )
        embed.add_field(name="💀 Deaths", value=f"**{guild["overall"]["deaths"]:,}**")
        embed.add_field(
            name="💀📚 Death fame", value=f"**{guild["guild"]["DeathFame"]:,}**"
        )
        embed.set_author(
            name=f"Requested by {interaction.user.name}",
            icon_url=interaction.user.avatar,
