...
class Info(Cog):
    ...
    async def gold(self, interaction: Interaction, count: int = 3) -> None:
        ...
        for i in range(len(data) - 1):
            embed.add_field(
                name=f"⌛ Data from {convert_api_timestamp(data[i]["timestamp"])}.",
    ...
    @command(
        name="premium", description="Retrieves price of premium status in the game."
    )
    @guild_only()
    async def premium(self, interaction: Interaction) -> None:
        ...
        embed: Embed = Embed(title="👑 Premium status price", color=Color.gold())
    ...
    async def price(self, interaction: Interaction, item_name: str) -> None:
        ...
        if not await view.wait():
            ...
            embed: Embed = Embed(
                title=f"🔘 {format_name(item_name)} price", color=Color.blurple()
            )
            embed.set_thumbnail(
            ...
            for entry in data:
                description.append(
                    f"**{entry["city"]}** (updated {convert_api_timestamp(entry["sell_price_min_date"])}): **{entry["sell_price_min"]:,}**\n"
                )

            description.append("\n***Buy orders:***\n")
            for entry in data:
                description.append(
                    f"**{entry["city"]}** (updated {convert_api_timestamp(entry["sell_price_min_date"])}): **{entry["buy_price_max"]:,}**\n"
                )
    ...
    async def utc(self, interaction: Interaction) -> None:
        await interaction.response.send_message(
            f"Current UTC time: {datetime.now(timezone.utc).strftime("%b %d, %Y. %H:%M:%S")}"
        )
