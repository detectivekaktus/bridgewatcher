...
class Settings(Cog):
    ...
    @choices(
        server=[
            Choice(name=name.capitalize(), value=name)
            for name in ("america", "europe", "asia")
        ]
    )
    @guild_only()
        ...
        match fetch_server:
            case 1:
                await interaction.response.send_message(
                    "Server successfully changed to 🇺🇸 America."
                )
            case 2:
                await interaction.response.send_message(
                    "Server successfully changed to 🇪🇺 Europe."
                )
            case 3:
                await interaction.response.send_message(
                    "Server successfully changed to 🇨🇳 Asia."
                )
