...
class Calcs(Cog):
    ...
    @command(
        name="craft", description="Calculates crafting profit from crafting an item"
    )
    ...
    async def craft(
        ...
        if item_name not in ITEM_NAMES.keys():
            await interaction.response.send_message(
                embed=NameErrorEmbed(item_name), ephemeral=True
            )
            return
        ...
        if not ItemManager.is_craftable(DATABASE, ITEM_NAMES[item_name]):
            await interaction.response.send_message(
                embed=Embed(
                    title=f"🔴 {format_name(item_name)} is not craftable!",
                    color=Color.red(),
        ...
        view: CraftingView = CraftingView(
            ITEM_NAMES[item_name],
            ItemManager.is_enchanted(ITEM_NAMES[item_name]),
            timeout=120,
        )
        await interaction.response.send_message(
            embed=Embed(
                title="🛠️ Crafting calculator",
                color=Color.magenta(),
        ...
        if not await view.wait():
            ...
            for resource in view.crafting_requirements.keys():
                global_resource_data: list[dict[str, Any]] | None = await MANAGER.get(
                    resource, server
                )
                if not global_resource_data:
                    ...
                    return
                resource_data: dict[str, Any] = get_city_data(
                    global_resource_data, craft_city
                )
                if resource_data["sell_price_min"] == 0:
                    await interaction.followup.send(
                        embed=OutdatedDataErrorEmbed(), ephemeral=True
                    )
                    return
            ...
            crafter: Crafter = Crafter(
                resource_prices,
                view.resources if view.resources else view.crafting_requirements,
                view.crafting_requirements,
                view.return_rate,
                has_premium,
            )
            ...
            embed.set_footer(
                text=f"The data is provided by the Albion Online Data Project | {inttoemoji_server(server)} server."
            )
            for field in result["fields"]:
                embed.add_field(name=field["title"], value=f"**{field["value"]}**")
            for unused_materials in result["unused_materials"]:
                embed.add_field(
                    name=unused_materials["name"],
                    value=f"**{unused_materials["value"]}**",
                )
    ...
    @command(
        name="flip",
        description="Calculates the profit of transportation of one item from city you select to the black market.",
    )
    @describe(item_name="The Albion Online item name.")
    @guild_only()
    async def flip(
        ...
        if not ItemManager.is_sellable_on_black_market(DATABASE, ITEM_NAMES[item_name]):
            await interaction.response.send_message(
                embed=Embed(
                    title=f"🔴 {format_name(item_name)} is not sellable on the black market!",
                    color=Color.red(),
        ...
        if not await view.wait():
            ...
            data = [
                get_city_data(data, view.cities[1]),
                get_city_data(data, view.cities[0]),
            ]
            ...
            if not data[0]["sell_price_min"] or not data[1]["sell_price_min"]:
                ...
                return
            tax: int = int(
                data[0]["sell_price_min"]
                * (PREMIUM_TAX if has_premium else NON_PREMIUM_TAX)
                / 100
            )

            embed: Embed = Embed(
                title=f"📦 Flipping the market for {format_name(item_name)}",
                color=Color.orange(),
            ...
            embed.add_field(
                name="💲 Buy price", value=f"**{data[1]["sell_price_min"]:,}**"
            )
            embed.add_field(
                name="💸 Sell price", value=f"**{data[0]["sell_price_min"]:,}**"
            )
