...
class ResourcesModal(Modal):
    def __init__(
        ...
        *,
        ...
        custom_id: str = "resources",
    ) -> None:
        super().__init__(title=title, timeout=timeout, custom_id=custom_id)
        self.view = view

        item_name: str = remove_suffix(
            DATABASE, self.view.item_name, self.view.is_enchanted
        )
        with DATABASE as db:
        ...
        for requirement in requirements:
            txt_input = TextInput(
                label=api_name_to_readable_name(ITEM_NAMES, requirement["@uniquename"]),
                placeholder=choice(["Eg. 100", "Eg. 3350", "Eg. 305", "Eg. 5000"]),
            )
        ...
        for txt_input in self.txt_inputs:
            try:
                ...
                await interaction.response.send_message(
                    "Some of your resource values are not valid.",
                    ephemeral=True,
                    delete_after=5,
                )
                return
...
class ReturnModal(Modal):
    def __init__(
        ...
        view: Any,
        *,
        title: str = "Return rate",
        timeout: Optional[float] = None,
        custom_id: str = "rrate",
    ) -> None:
        super().__init__(title=title, timeout=timeout, custom_id=custom_id)
        self.view = view
        self.return_rate: TextInput = TextInput(
            label=title, placeholder="Eg. 15 or 25.5"
        )
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        try:
            ...
            if return_rate >= 100 or return_rate < 0:
                raise ValueError(
                    "Return rate can't be more than 99 percent and can't be below 0 percent."
                )
            self.view.return_rate = return_rate
            await interaction.response.defer()
        except ValueError:
            await interaction.response.send_message(
                f"{self.return_rate.value} is not a valid return rate.",
                ephemeral=True,
                delete_after=5,
            )
            return
